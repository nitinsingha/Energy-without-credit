...
m = 15;        % Number of buyers
n = 45;        % Number of sellers
%m_mal=15;      % Numner of buyers who have entered into mailicious agreemnet with central authority to get all the energy
...
while flag == 1
    ...
    % Introducing malicious buyers behaviour in the network
    %C(1:m_mal,:)=(C_max/n).*ones(m_mal,n);
    %C(m_mal+1:m,:)=zeros(m-m_mal,n);
    C(1:m,:)=(C_max/n).*ones(m,n);
    D=C/rho;    % The energy that is being transmitted from sellers to consumers with some transmission losses.
...
social_welfare = sw(find(sw,1,'last'))
sD = sum(D,2);
save('saveD.mat','sD');
save('data_buyer_210722.mat');
% Plots
% figure
% plot(sw(1:end), '-b')
