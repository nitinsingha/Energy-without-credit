...
function [node] = trade_not_present(node,seller,buyers_id,index,current_request,iter)
...
if index == -1   % The current buyer lies lowest in the preference list, i.e., it is present at the end
    ...
    if remaining >0
        %update request
        request = updateRequest_strucutre(seller,buyers_id,remaining,node(seller).bidPrice,node(buyers_id).bidPrice,iter);   % Creteas a request stucture for the request
        %update buyer
