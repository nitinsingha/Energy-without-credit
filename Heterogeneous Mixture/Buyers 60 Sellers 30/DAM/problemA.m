...
D = optimvar('y',n,m,'LowerBound',0);     % Electricity supply by discharging vehicles
...
prob.Constraints.cons3 = (sum(D,2))<=D_max;          % problem. In the paper,
prob.Constraints.cons4 = (rho*D == transpose(C));                         % they are written in eqn. (4).


x0.x = ones(m,n);
x0.y = ones(n,m);

options = optimoptions(@fmincon,'MaxFunctionEvaluations',5000); % Solving the optimisation problem
...
function y = objective(C,D,B,S)                              % Objective function that has to be optimised. Eqn (10) in the paper.
    y = -(sum(sum(dot(B,log(C)) - transpose(dot(S,D)))));
end
end