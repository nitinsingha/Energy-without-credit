...
function [node]=sendRequest(node,buyer,iter)
...
% 2. It submits the request to the seller. Reqquest is send to seller in form of strucutre called request
        ...
        request =  updateRequest_strucutre(bid_sel,buyer,blocks_requested,node(bid_sel).bidPrice,node(buyer).bidPrice,iter);
