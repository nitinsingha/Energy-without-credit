...
% This function updates cost at which DSO sells and purchases blocks 
 ...
 [D,C,B,S,rewd]=getData();           
    ...
    avgC =  round(mean(sum(C,2)));      % Average supply by sellers. The average supply is rounded to the nearest integer.
    avgB =  (mean(sum(B,2)));      % Average bid of a buyer, for all the blocks
    avgS =  (mean(sum(S,2)));      % Average bid by a seller, for all the blocks.
    ...
    DSO_block_requisiton_cost = (avgS-2*(avgS/10))/avgD;  % This is minimum bid by a seller
end
