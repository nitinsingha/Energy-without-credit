...
if index == -1   % The current buyer lies lowest in the preference list, i.e., it is present at the end
    ...
    if remaining >0
        %update request
        request = updateRequest_strucutre(seller,buyers_id,remaining,node(seller).bidPrice,node(buyers_id).bidPrice);   % Creteas a request stucture for the request
        %update buyer
        ...
        buyers_list = [buyers_list;request];
    ...
    if min_sold >0
        ...
        %update request
        request = updateRequest_strucutre(seller,buyers_id,min_sold,node(seller).bidPrice,node(buyers_id).bidPrice);   % Creteas a request stucture for the request
        %update buyer
        isPresent=0; % The buyer has never requested seller before. So buyer is not present in mapping list of seller.
        node = updateBuyer_addition(node,isPresent,request);
        ...
        %update buyers_list
        buyers_list = [buyers_list(1:index-1);request;buyers_list(index:end)];
        ...
        if temp_index >0
            ...
            temp_request = updateRequest_strucutre(seller,buyers_list(temp_index).buyers_id,blocks_removed,node(seller).bidPrice,node(buyers_list(temp_index).buyers_id).bidPrice);   % Creteas a request stucture for the request
            ...
            for i = temp_index+1:length(buyers_list)
                ...
                temp_request = updateRequest_strucutre(seller,buyers_list(i).buyers_id,blocks_removed,node(seller).bidPrice,node(buyers_list(i).buyers_id).bidPrice);   % Creteas a request stucture for the request
                node = updateBuyer_removal(node,temp_request);
            end
