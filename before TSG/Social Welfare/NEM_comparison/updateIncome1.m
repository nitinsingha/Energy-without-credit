function [node]=updateIncome1(node)
...
for itr = 1:1:n
    ...
    if(node(itr).type==0)   % Buyer
        list = node(itr).final_mapping; % The details about all the transactions made by a buyer are stored in strucutr mapping. Refer file "creating_buyers.m" for details.
    ...
    elseif (node(itr).type==1)  % Seller
        list = node(itr).final_buyers_list; % The details about all the transactions made by seller are stored in strucutr buyers_list. Refer file "creating_sellers.m" for details.
        for itr1 =1:1:length(list)
            node(itr).income_round = node(itr).income_round + list(itr1).blocks*list(itr1).selling_price; 
        end  
    end 
